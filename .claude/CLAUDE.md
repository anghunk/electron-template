...
这是一个使用 Vue 3、Vite、Element Plus 和 Tailwind CSS 的现代 Electron 桌面应用程序模板。该应用程序具有暖色调且优雅的 UI 设计，包括自定义无边框窗口、托盘集成、设置管理和**暗黑模式切换**功能。
...
- **默认**：`{ closeToTray: true, darkMode: false }`
- **功能**：
  - 关闭到托盘行为切换
  - **暗黑模式切换**（新增）
  - 在应用程序重启后持续存在
...
- **管理器**：`src/utils/themeManager.js` - 单例模式管理主题
- **切换按钮**：侧边栏左下角（PhMoon/PhSun 图标）
- **跟随系统**：右键点击或点击显示器图标可切换跟随系统模式
- **CSS 变量**：`:root` 和 `:root.dark` 定义两套主题
- **持久化**：通过 Electron 设置 API 保存
- **系统检测**：首次启动检测 `prefers-color-scheme`
- **实时监听**：跟随系统模式下监听系统主题变化
- **图标系统**：主按钮根据状态显示不同图标（月亮/太阳/显示器）
- **平滑过渡**：0.3s 颜色过渡动画
- **滚动条**：暗黑模式下有专门的深色滚动条样式
...
**使用规则**：
- **左键点击主按钮**：切换浅色/暗黑模式（自动退出跟随系统）
- **右键点击主按钮**：切换跟随系统开关
- **点击显示器按钮**：快速切换跟随系统
- **跟随系统**：自动根据系统主题切换，手动切换时自动退出
